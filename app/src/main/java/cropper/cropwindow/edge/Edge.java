...
package cropper.cropwindow.edge;

import android.graphics.Rect;
import android.view.View;

import cropper.util.AspectRatioUtil;
...
public enum Edge {
    ...
    public void adjustCoordinate(float aspectRatio) {
        ...
        final float top = cropper.cropwindow.edge.Edge.TOP.getCoordinate();
        final float right = cropper.cropwindow.edge.Edge.RIGHT.getCoordinate();
        final float bottom = cropper.cropwindow.edge.Edge.BOTTOM.getCoordinate();
     ...
     */
    public boolean isNewRectangleOutOfBounds(cropper.cropwindow.edge.Edge edge, Rect imageRect, float aspectRatio) {

        float offset = edge.snapOffset(imageRect);
        
        switch (this) {
            case LEFT:
                if (edge.equals(cropper.cropwindow.edge.Edge.TOP)) {
                    float top = imageRect.top;
                    float bottom = cropper.cropwindow.edge.Edge.BOTTOM.getCoordinate() - offset;
                ...
                else if (edge.equals(cropper.cropwindow.edge.Edge.BOTTOM)) {
                    float bottom = imageRect.bottom;
                    float top = cropper.cropwindow.edge.Edge.TOP.getCoordinate() - offset;
                    float right = cropper.cropwindow.edge.Edge.RIGHT.getCoordinate();
                    float left = AspectRatioUtil.calculateLeft(top, right, bottom, aspectRatio);
            ...
            case TOP:
                if (edge.equals(cropper.cropwindow.edge.Edge.LEFT)) {
                    float left = imageRect.left;
                    float right = cropper.cropwindow.edge.Edge.RIGHT.getCoordinate() - offset;
                ...
                else if (edge.equals(cropper.cropwindow.edge.Edge.RIGHT)) {
                    float right = imageRect.right;
                    float left = cropper.cropwindow.edge.Edge.LEFT.getCoordinate() - offset;
                    float bottom = cropper.cropwindow.edge.Edge.BOTTOM.getCoordinate();
                    float top = AspectRatioUtil.calculateTop(left, right, bottom, aspectRatio);
    ...
    */
    ...
    public static float getWidth() {
        return cropper.cropwindow.edge.Edge.RIGHT.getCoordinate() - cropper.cropwindow.edge.Edge.LEFT.getCoordinate();
    }
    ...
    public static float getHeight() {
        return cropper.cropwindow.edge.Edge.BOTTOM.getCoordinate() - cropper.cropwindow.edge.Edge.TOP.getCoordinate();
    }
    ...
    private static float adjustLeft(float x, Rect imageRect, float imageSnapRadius, float aspectRatio) {
        ...
        else
        {
            ...
            if (x >= cropper.cropwindow.edge.Edge.RIGHT.getCoordinate() - MIN_CROP_LENGTH_PX)
                resultXHoriz = cropper.cropwindow.edge.Edge.RIGHT.getCoordinate() - MIN_CROP_LENGTH_PX;

            // Checks if the window is too small vertically
            if (((cropper.cropwindow.edge.Edge.RIGHT.getCoordinate() - x) / aspectRatio) <= MIN_CROP_LENGTH_PX)
                resultXVert = cropper.cropwindow.edge.Edge.RIGHT.getCoordinate() - (MIN_CROP_LENGTH_PX * aspectRatio);
    ...
    private static float adjustRight(float x, Rect imageRect, float imageSnapRadius, float aspectRatio) {
        ...
        else
        {
            ...
            // Checks if the window is too small horizontally
            if (x <= cropper.cropwindow.edge.Edge.LEFT.getCoordinate() + MIN_CROP_LENGTH_PX)
                resultXHoriz = cropper.cropwindow.edge.Edge.LEFT.getCoordinate() + MIN_CROP_LENGTH_PX;

            // Checks if the window is too small vertically
            if (((x - cropper.cropwindow.edge.Edge.LEFT.getCoordinate()) / aspectRatio) <= MIN_CROP_LENGTH_PX) {
                resultXVert = cropper.cropwindow.edge.Edge.LEFT.getCoordinate() + (MIN_CROP_LENGTH_PX * aspectRatio);
            }
    ...
    private static float adjustTop(float y, Rect imageRect, float imageSnapRadius, float aspectRatio) {
        ...
        else
        {
            ...
            // Checks if the window is too small vertically
            if (y >= cropper.cropwindow.edge.Edge.BOTTOM.getCoordinate() - MIN_CROP_LENGTH_PX)
                resultYHoriz = cropper.cropwindow.edge.Edge.BOTTOM.getCoordinate() - MIN_CROP_LENGTH_PX;

            // Checks if the window is too small horizontally
            if (((cropper.cropwindow.edge.Edge.BOTTOM.getCoordinate() - y) * aspectRatio) <= MIN_CROP_LENGTH_PX)
                resultYVert = cropper.cropwindow.edge.Edge.BOTTOM.getCoordinate() - (MIN_CROP_LENGTH_PX / aspectRatio);
    ...
    private static float adjustBottom(float y, Rect imageRect, float imageSnapRadius, float aspectRatio) {
        ...
        else
        {
            ...
            // Checks if the window is too small vertically
            if (y <= cropper.cropwindow.edge.Edge.TOP.getCoordinate() + MIN_CROP_LENGTH_PX)
                resultYVert = cropper.cropwindow.edge.Edge.TOP.getCoordinate() + MIN_CROP_LENGTH_PX;

            // Checks if the window is too small horizontally
            if (((y - cropper.cropwindow.edge.Edge.TOP.getCoordinate()) * aspectRatio) <= MIN_CROP_LENGTH_PX)
                resultYHoriz = cropper.cropwindow.edge.Edge.TOP.getCoordinate() + (MIN_CROP_LENGTH_PX / aspectRatio);
